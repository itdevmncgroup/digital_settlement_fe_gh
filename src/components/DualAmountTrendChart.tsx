...
const SERIES = [
  { key: 'totalExpense' as const, label: 'Total Expenses', color: '#8b7bfb' },
  { key: 'settled' as const, label: 'Settlement', color: '#3fd085' },
];

// Two money-scale lines (Total Expenses vs Settlement) sharing one axis, each
// point labeled with its compact amount - "Monthly Expense vs Settlement
// Trend" panel.
export default function DualAmountTrendChart({ data }: { data: TrendRow[] }) {
  ...
  const max = Math.max(...data.flatMap((d) => [d.totalExpense, d.settled]), 1);
  ...
  const linesData = SERIES.map((s) => {
    const points = data.map((d, i) => {
      ...
      const y = PAD_TOP + plotHeight - (d[s.key] / max) * plotHeight;
      return { x, y, value: d[s.key] };
    });
